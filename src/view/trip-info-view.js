...
function getTripTitle(events) {
  const destinations = [...new Set(events.map((event) => getEventDestination(event).name))];
  let title = '';

  destinations.forEach((destination, index) => {
    title += index !== destinations.length - 1 ? `${destination} &mdash; ` : destination;
  });
...
function getTripInterval(events) {
  let interval = '';
  interval += `${humanizeDate(events[0].dateFrom, DATE_FORMAT_DAY)}&nbsp;&mdash;&nbsp;`;
  interval += `${humanizeDate(events.slice(-1)[0].dateTo, DATE_FORMAT_DAY).split(' ')[1]}`;
  return interval;
}

function getTripCost(events) {
  const cost = events.reduce((totalCost, event) => totalCost + event.basePrice, 0);
  return cost;
}
